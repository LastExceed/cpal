use super::*;
use azo::sys::Callbacks as Pointers;
use azo::sys::{
    AsioMessage, Bool, BufferSwitch, BufferSwitchTimeInfo, MessageSelector, SampleRateDidChange,
    Time,
};
...
impl Callbacks {
    ...
    pub fn prime(
        ...
        // SAFETY:
        // The closures relying on this pin are (re-)created here
        let mutable = unsafe { Pin::get_unchecked_mut(self) };

        let error_cb1 = error_cb.pipe(Mutex::new).pipe(Arc::new);
        ...
        mutable.closures.buffer_switch_time_info =
            create_buffer_switch_time_info(error_cb4, data_cb, simplex_in, simplex_out);
        mutable.closures.buffer_switch = create_buffer_switch(
            error_cb1,
            session,
            mutable.closures.buffer_switch_time_info.bare(),
        );
...
pub fn create_buffer_switch(
    ...
    let closure =
        move |buf_idx: c_long, direct_process: Bool| match session.driver.sample_position() {
            Ok(pos) => {
                let mut time = create_minimal_asio_time(&pos);
                unsafe {
                    bsti_ptr(&raw mut time, buf_idx, direct_process);
                }
            }
            Err(error) => throw(
                &error_cb,
...
pub fn create_asio_message(error_cb: Arc<Mutex<error_cb_type!()>>) -> Bare<AsioMessage> {
    let closure = move |selector, value, _message, _opt| match selector {
        MessageSelector::SELECTOR_SUPPORTED => {
        ...
        MessageSelector::RESET_REQUEST => {
            throw(
                &error_cb,
                Error::with_message(StreamInvalidated, "ASIO driver requested a reset"),
            );
            Bool::TRUE.0
        }
        ...
        MessageSelector::RESYNC_REQUEST => {
            throw(
                &error_cb,
                Error::with_message(StreamInvalidated, "ASIO driver requested a resync"),
            );
            Bool::TRUE.0
        }
...
pub fn create_buffer_switch_time_info(
    ...
    let closure = move |time: *mut Time, buf_idx: c_long, direct_process: Bool| {
        ...
        let callback_info = unsafe { time.read() }
            .time_info
            .system_time
            .pipe(|time| StreamInstant::from_millis(time as _))
            .pipe(|instant| StreamTimestamp {
                callback: instant,
                device: instant,
            })
        ...
        data_callback(
            &simplex_in.data(buf_idx as _),
            &mut simplex_out.data(buf_idx as _),
            &callback_info,
        );
