...
impl DeviceTrait for Device {
        ...
        ErrorCb: FnMut(Error) + Send + 'static,
    {
        ...
        self.0
            .build_stream(cfg_in, cfg_out, sample_rate, buffer_size, data_cb, error_cb)
    }
}
...
impl Drop for Stream {
    fn drop(&mut self) {
        ...
        _ = self.session.driver.dispose_all_buffers(); // if something important goes wrong here, the driver will keep complaining in subsequent interactions
        *self.session.stream_exists.lock().expect("mutex poisoned") = false;
    }
}
